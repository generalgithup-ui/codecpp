...
#include <string>
#include <iomanip>
#include <limits> // ??????????? numeric_limits ??????????? Buffer ?????????
...
public:
    ...
    bool login(string inputEmail, string inputPw) const {
...
public:
    ...
    void takeOrder() {
        ...
        do {
            ...
            while (!(cin >> index) || index < 0 || index >= itemCount) {
                cin.clear();
                cin.ignore(numeric_limits<streamsize>::max(), '\n');
                cout << ">> Invalid item number! Please enter between 0 and 7: ";
            }
